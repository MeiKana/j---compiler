...
public class Parser {
    ...
    private JInterfaceDeclaration interfaceDeclaration(){
        int line = scanner.token().line();
        mustBe(INTERFACE);
        mustBe(IDENTIFIER);
        ...
        return new JInterfaceDeclaration(line, name, superClass, interfaceBody());
    }
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        ArrayList<Type> implementsList = new ArrayList<Type>();
        if (have(EXTENDS)) {
            superClass = qualifiedIdentifier();
        } else if (have(IMPLEMENTS)){
            do{
                implementsList.add(qualifiedIdentifier());
            } while(have(COMMA));
        }
        else {
            superClass = Type.OBJECT;
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            if (have(VOID)) {
                ...
                memberDecl = (hasException? (new JMethodDeclaration(line,
                         mods, name, type, params, body, exceptions)):
                         (new JMethodDeclaration(line, mods, name, type,
                            params, body)));
            } else {
                type = type();
                if (seeIdentLParen()) {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            Object forInitDecl = new Object();
            Object forInitStatement = new Object();
            if (seeBasicType()){
                forInitDecl = localVariableDeclarationStatement();
                isDecl = true;
            }else if (have(SEMI)){
                forInitStatement = new JEmptyStatement(line);
            }
            else{
...
//            mustBe(SEMI);
            ...
            if(see(SEMI))
                forStatement = new JEmptyStatement(line);
            else
                forStatement = statement();
            return new JForStatement(line, (isDecl? (JVariableDeclaration) 
                                            forInitDecl: (JStatement) forInitStatement),
                                            test, update, forStatement);

        } else if (have(DO)){
            JStatement doBody = statement();
            mustBe(WHILE);
            ...
            ArrayList<JFormalParameter> catchParas = new ArrayList<JFormalParameter>();
            while(have(CATCH)){
                mustBe(LPAREN);
                catchParas.add(formalParameter());
                mustBe(RPAREN);
                catchBlocks.add(block());
            }
            if(have(FINALLY)){
                return new JTryCatchFinallyStatement(line, tryBlock, catchBlocks,
                                        catchParas, block());
            }
            else
                return new JTryCatchFinallyStatement(line, tryBlock, catchBlocks,
            ...
            JExpression condition = parExpression();
            Hashtable<Integer,JExpression> caseList = new Hashtable<Integer,JExpression>();
            Hashtable<Integer,ArrayList<JStatement>> stateList = new Hashtable<Integer,ArrayList<JStatement>>();
            int caseCounter = 0;
            mustBe(LCURLY);
            while(!have(RCURLY)){
                if(have(CASE)){
                    caseCounter++;
                    caseList.put(caseCounter, expression());
                    mustBe(COLON);
                }
                    ...
                    stateList.put(caseCounter, statements);
                }
            }
            return new JSwitchStatement(line, condition, caseList, stateList, caseCounter);
        }else if (have(BREAK)){
            JStatement statement = new JBreakStatement(line);
            mustBe(SEMI);
            return statement;
        }else if (have(CONTINUE)){
            JStatement statement = new JContinueStatement(line);
            mustBe(SEMI);
            return statement;
        }
    ...
    private JExpression conditionalExpression(){
        ...
        if(have(COND)){
            JExpression trueExp = expression();
            mustBe(COLON);
            JExpression falseExp = expression();
            return new JConditionalExpression(line, lhs, trueExp, falseExp);
        }else
            return lhs;
    ...
    private JExpression relationalExpression() {
        ...
        } else if (have(GREAT_EQUAL)){
            return new JGreaterEqualOp(line, lhs, ShiftExpression());
        } else if (have(SMALLER)){
            return new JLesserOp(line, lhs, ShiftExpression());
        }
    ...
    private JExpression ShiftExpression(){
        ...
        while(more){
            ...
            else if (have (U_R_SHFIT)){
                lhs = new JUnsignedRightShiftOp(line, lhs, additiveExpression());
            }
            else if (have(U_L_SHIFT)){
                lhs = new JUnsignedLeftShiftOp(line, lhs, additiveExpression());
            }
    ...
    private JExpression simpleUnaryExpression() {
        ...
        if (have(LNOT)) {
            ...
            return new JBitNotOp(line, unaryExpression()); 
        }
        else {
